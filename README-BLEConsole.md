...
`BLEConsole` can run as a local [Model Context Protocol](https://modelcontextprotocol.io) server, so
an AI agent can scan, connect to and talk to BLE devices with the same command set described below.

```powershell
msbuild BLEConsole\BLEConsole.csproj /p:Configuration=Release /p:Platform=AnyCPU
node mcp-server\smoke-test.mjs      # verify
```

Add `node <repo>\mcp-server\server.mjs` as a stdio MCP server in your client (Claude Desktop,
Cursor, DSH, …). The server drives a long-lived `BLEConsole.exe --mcp` child process, because the
connection and the notification subscriptions are process state. 22 tools are exposed, covering
device discovery, GATT read/write/subscribe, descriptors, pairing, MTU and session configuration.

See the [main README](README.md) for configuration, the full tool list and troubleshooting.
